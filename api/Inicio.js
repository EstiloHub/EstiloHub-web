...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const faseUsuario =
      String(usuario.fase || "Fase 1").trim();
    ...
    tareasSnapshot.forEach((documento) => {
      ...
      const faseTarea =
        String(datos.fase ?? "").trim();


      const correspondeFase =
        faseTarea === faseUsuario ||
        faseTarea === "1" && faseUsuario === "Fase 1" ||
        faseTarea === "Fase 1" && faseUsuario === "1";
      ...
      tareas.push({
        ...
        tiempo_requerido:
          Number(datos.tiempo_requerido || 0),

        link_url:
          String(datos.link_url || ""),

        orden:
          Number.isFinite(Number(datos.orden))
            ? Number(datos.orden)
            : 999999
    ...
    progresoSnapshot.forEach((documento) => {
      ...
      progreso[datos.tarea_id] = {
        ...
        fecha_realizada:
          datos.fecha_realizada || null,

        minutos_realizados:
          Number(datos.minutos_realizados || 0),

        veces_realizada:
          Number(datos.veces_realizada || 0)
    ...
    const tareasParaCliente =
      tareas.map((tarea) => {
        ...
        return {
          ...
          fecha_realizada:
            datosProgreso.fecha_realizada || null,

          minutos_realizados:
            datosProgreso.minutos_realizados || 0,

          veces_realizada:
            datosProgreso.veces_realizada || 0
