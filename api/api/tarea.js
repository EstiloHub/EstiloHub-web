...
async function obtenerTareaYUsuario(uid, tareaId) {
  ...
  if (String(tarea.fase || "") !== String(usuario.fase || "")) {
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      const tareaId = String(req.query.id || "").trim();
      ...
      const { tarea } = await obtenerTareaYUsuario(uid, tareaId);

      const sesionId = `${uid}_${tareaId}`;
      const sesionRef = db.collection("sesiones_tareas").doc(sesionId);
      ...
      if (sesionSnap.exists) {
        ...
        tiempoAcumulado = Number(sesion.tiempo_acumulado || 0);
        activa = sesion.activa === true;

        if (sesion.inicio_actual) {
          inicioActual = sesion.inicio_actual.toMillis();
        }
        ...
        if (activa && inicioActual) {
          ...
          tiempoAcumulado += Math.max(0, tiempoDesdeInicio);
        }
      }

      const tiempoRequerido =
        Number(tarea.tiempo_requerido || 0) * 60 * 1000;
    ...
    if (req.method === "POST") {
      const tareaId = String(req.body?.tarea_id || "").trim();
      ...
      const { tarea } =
        await obtenerTareaYUsuario(uid, tareaId);

      const sesionId = `${uid}_${tareaId}`;
      const sesionRef =
        db.collection("sesiones_tareas").doc(sesionId);
      ...
      if (sesionSnap.exists) {
        ...
        tiempoAcumulado =
          Number(sesion.tiempo_acumulado || 0);
          ...
          await sesionRef.set(
            {
              ...
              activa: true,
              inicio_actual: FieldValue.serverTimestamp(),
      ...
      if (accion === "pausar") {
        if (activa && inicioActual) {
          ...
          const inicio = inicioActual.toMillis();

          tiempoAcumulado += Math.max(
            0,
            ahora - inicio
          );
        }

        tiempoAcumulado = Math.min(
          tiempoAcumulado,
          tiempoRequerido
        );
      ...
      if (accion === "completar") {
        ...
        if (tiempoAcumulado < tiempoRequerido) {
          ...
          return res.status(400).json({
            ok: false,
            error: "Todavía no cumpliste el tiempo requerido.",
            tiempo_acumulado: tiempoAcumulado
          });
        }
        ...
        await db.runTransaction(async (transaction) => {
          const progresoSnap =
            await transaction.get(progresoRef);

          const estadisticasSnap =
            await transaction.get(estadisticasRef);

          if (progresoSnap.exists) {
            transaction.update(progresoRef, {
              veces_realizada: FieldValue.increment(1),
              fecha_realizada: FieldValue.serverTimestamp(),
          ...
          transaction.set(
            sesionRef,
            {
              usuario_id: uid,
              tarea_id: tareaId,
              tiempo_acumulado: tiempoRequerido,
              activa: false,
              inicio_actual: null,
              completada: true,
              fecha_completada:
